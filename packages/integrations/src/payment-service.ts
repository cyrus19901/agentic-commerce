...
export class PaymentService {
  ...
  async initiateCheckout(request: CheckoutRequest): Promise<CheckoutResponse> {
    ...
    try {
      ...
      const session = await this.stripe.checkout.sessions.create({
        ...
        mode: 'payment',
        success_url: `${process.env.API_URL || process.env.RENDER_EXTERNAL_URL || 'http://localhost:3000'}/checkout/success?session_id={CHECKOUT_SESSION_ID}`,
        cancel_url: `${process.env.API_URL || process.env.RENDER_EXTERNAL_URL || 'http://localhost:3000'}/checkout/cancel`,
        metadata: {
          userId: request.userId,
      ...
      return {
        ...
        invoiceUrl: `${process.env.API_URL || process.env.RENDER_EXTERNAL_URL || 'http://localhost:3000'}/api/invoices/${session.id}`,
        productId: request.productId,
        productName: request.productName,
        amount: request.amount,
        currency: 'USD',
  ...
  private getMockCheckoutResponse(request: CheckoutRequest): CheckoutResponse {
    ...
    return {
      ...
      invoiceUrl: `${process.env.API_URL || process.env.RENDER_EXTERNAL_URL || 'http://localhost:3000'}/api/invoices/${mockSessionId}`,
      productId: request.productId,
      productName: request.productName,
      amount: request.amount,
      currency: 'USD',
